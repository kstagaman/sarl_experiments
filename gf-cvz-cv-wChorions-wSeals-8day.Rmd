...
stats_table2[, activity := ifelse(`significance p<0.01` == "YES" & RelativeRatio <= -0.1, "HYPO", NA)]
...
dat <- as.data.frame(behav5d)
c1 <- dat[which(dat$Treatment == ctrl.treat),]
...
dat <- copy(behav5d)
c1 <- dat[Treatment == ctrl.treat]
c1.clean <- c1[complete.cases(c1)]
# cl_all_aucs <- 
# c1.clean <- as.data.frame(c1.clean)
# apply(c1.clean[,match(c(dark_t,light_t), names(c1.clean))], 2, trapz)
c1.trapz <- c1.clean[, c(lapply(.SD, function(col) caTools::trapz(x = 1:nrow(c1.clean), y = col))), .SDcols = c(dark_t, light_t)]
c1.sum <- c1.clean[, c(lapply(.SD, sum)), .SDcols = c(dark_t, light_t)]
identical(c1.trapz, c1.sum)
View(rbind(c1.trapz, c1.sum))
c1.mean <- c1.clean[, c(lapply(.SD, mean)), .SDcols = c(dark_t, light_t)]
```
