...
libs <- c("data.table", "magrittr", "ggplot2", "cowplot", "ggbeeswarm", "caTools", "FSA", "ggsignif", "kableExtra", "RColorBrewer", "LaCroixColoR")
...
wrap.caption <- function(caption, w = 100) {
  split.cap <- strwrap(caption, width = w)
  wrapped.cap <- paste(split.cap, collapse = "\n")
  return(wrapped.cap)
}
create.caption <- function(caption) {
  figure.env$count <- figure.env$count + 1
  cap <- paste0("Figure ", figure.env$count, ". ", caption)
  return(wrap.caption(cap))
}
# three_colors <- scale_color_brewer(name = "Treatment", palette = "Dark2")
base.cols <- lacroix_palettes$Apricot[1, c(4, 2, 6)]
three_colors <- scale_color_manual(name = "Treatment", values = base.cols)
# three_fills <- scale_fill_brewer(name = "Treatment", palette = "Dark2")
three_fills <- scale_fill_manual(name = "Treatment", values = base.cols)
...
my.pairs <- c(
  base.cols[1], lacroix_palettes$Pure[1, 1], 
  base.cols[2], lacroix_palettes$PommeBaya[1, 2],
  base.cols[3], lacroix_palettes$PeachPear[1, 5]
  )
plate_id_colors <- scale_color_manual(name = "Plate", values = my.pairs[c(5, 6, 3, 4, 1, 2)])
plate_id_new_colors <- scale_color_manual(name = "Plate", values = my.pairs)
plate_id_new_fills <- scale_fill_manual(name = "Plate", values = my.pairs)
my.yellow <- tail(RColorBrewer::brewer.pal(12, "Set3"), 1)
```


### Day 1 - HMAT

```{r hmat-all-movement-plot}
...
cap <- create.caption(
  "Plot of movement in microns over time. Lines indicate mean movement per treatment, ribbons indicate bootstrapped 95% CIs. Vertical red lines indicate the window of movement we will analyze statistically"
)
ggplot(hmat.melt, aes(x = variable, y = value)) +
  ...
  three_colors +
  three_fills + 
  labs(x = "Time", y = "Movement [µM]", title = "HMAT Results", caption = wrap.caption(cap))
```
...
kable(
  ...
  caption = "Example of AUC data used for statistics"
    ...
    kable_styling(bootstrap_options = "striped", font_size = tbl.font.size)
kable(
  aucs[, .N, by = Treatment],
...
cap <- create.caption(
  "Plot of AUCs for individual wells (larvae) within each Treatment, calculated using the trapezoid method. Post-hoc pairwise comparisons were conducted with Dunn's Kruskal-Wallis Multiple Comparisons Test with False Discovery Rate"
)
ggplot(aucs) +
  ...
  labs(title = "Individual HMAT AUCs", caption = cap) + 
  theme(legend.position = "none")
```

```{r hmat-plot-with-plates}
kable(
...
cap <- create.caption(
  "Plot of AUCs for individual wells (larvae) within each Plate, calculated using the trapezoid method."
)
...
light.seg.y <- floor(min(hcal4$h.sec))
cap <- create.caption(
  "LPR results from epoch 2 by Treatment, plotted according to same method as from SARL workflow. Each line represents the sum of the mean DE at each time point for each treatment [sum(mean(DE)) ~ Treatment + Sec]. The yellow line indicates light cycle (vs dark cycle)"
)
ggplot(hcal4, aes(sec, h.sec, colour = Treatment)) +
  geom_path(lwd = 1.2, aes(group = Treatment)) +
  geom_segment(x = 0, y = light.seg.y, xend = 150, yend = light.seg.y, color = my.yellow, size = 2) +
  # scale_color_brewer(name = "Treatment", palette = "Dark2") +
  three_colors + 
  labs(x = "Time (sec)", y = "Differential Entropy", caption = cap)
```

```{r lpr-ep2-by-treat-plot2}
hcal.plotdata <- hcal[variable %in% ep2]
hcal.plotdata[, time := as.numeric(gsub("t", "", as.character(variable)))]
hcal.plotdata[, sec := (floor((time - 61) / 5)) * 30]
light.seg.y <- floor(min(hcal.plotdata$h))
cap <- create.caption(
  "LPR results from epoch 2 by Treatment, plotted from raw DE values (which are used for calculating stats). Each line represents the mean DE at each time point for each treatment [mean(DE) ~ Treatment + Sec], and the shading indicates the bootstrapped 95% CI [smean.cl.boot()]. The yellow line indicates light cycle (vs dark cycle)"
)
ggplot(hcal.plotdata, aes(x = sec, y = h)) +
  stat_summary(fun.data = "mean_cl_boot", geom = "ribbon", aes(fill = Treatment), alpha = 0.2) +
  stat_summary(fun.y = mean, geom = "path", aes(color = Treatment)) + 
  geom_segment(x = 0, y = light.seg.y, xend = 150, yend = light.seg.y, color = my.yellow) +
  three_colors + 
  three_fills + 
  labs(x = "Time (sec)", y = "Differential Entropy", caption = cap) +
  coord_cartesian(ylim = c(light.seg.y, max(hcal.plotdata$h)))
```

I am unsure why there's the dip in DE for the Fig. 4 but not for Fig. 5. The math should be the same, the difference is that for Fig. 4, the mean and then sum of DE is calculated prior to plotting, whereas in Fig. 5 I let `ggplot` do the math for me with `stat_summary`. I'll look into this discrepancy. (Also see below in Figs. 7 and 8, the dip at the end is not there either.)
...
```{r lpr-ep2-by-plate-plot1}
hcal5 <- hcal.plotdata[, .(h.sec = sum(mean(h))), by = c("Treatment", "plate", "sec")]
hcal5[
  ,
  plate.id.new := factor(
    mgsub(
...
light.seg.y <- floor(min(hcal5$h.sec))
cap <- create.caption(
  "LPR results from epoch 2 by Treatment and Plate, plotted according to same method as from SARL workflow. Each line represents the sum of the mean DE at each time point for each treatment [sum(mean(DE)) ~ Treatment + Plate + Sec]. The yellow line indicates light cycle (vs dark cycle)"
)
ggplot(hcal5, aes(x = sec, y = h.sec, colour = plate.id.new)) +
  geom_path(lwd = 1.2, aes(group = plate)) +
  geom_segment(x = 0, y = light.seg.y, xend = 150, yend = light.seg.y, color = my.yellow, size = 2) +
  plate_id_new_colors + 
  labs(x = "Time", y = "Differential Entropy", caption = cap) +
  coord_cartesian(ylim = c(light.seg.y, max(hcal5$h.sec)))
```


```{r lpr-ep2-by-plate-plot2}
hcal.plotdata[
  ,
  plate.id.new := factor(
    mgsub(
...
light.seg.y <- floor(min(hcal.plotdata$h))
cap <- create.caption(
  "LPR results from epoch 2 by Treatment and Plate, plotted from raw DE values (which are used for calculating stats). Each line represents the mean DE at each time point for each treatment [mean(DE) ~ Treatment + Plate + Sec], and the shading indicates the bootstrapped 95% CI [smean.cl.boot()]. The yellow line indicates light cycle (vs dark cycle)"
)
ggplot(hcal.plotdata, aes(x = sec, y = h)) +
  # geom_path(lwd = 1.2, aes(group = Treatment)) +
  stat_summary(fun.data = "mean_cl_boot", geom = "ribbon", aes(fill = plate.id.new), alpha = 0.2) +
  stat_summary(fun.y = mean, geom = "path", aes(color = plate.id.new)) + 
  geom_segment(x = 0, y = light.seg.y, xend = 150, yend = light.seg.y, color = my.yellow, size = 2) +
  # scale_color_brewer(name = "Treatment", palette = "Dark2") +
  plate_id_new_colors + 
  plate_id_new_fills + 
...
cap <- create.caption(
  "LPR results from 2nd, 3rd, and 4th epochs. Lines indicate mean movement (raw) by Treatment, shading indicates the bootstrapped 95% CIs. Yellow segments at bottom indicate light intervals (vs dark intervals)."
)
ggplot(lpr.plot.data) +
  ...
  three_colors + 
  three_fills + 
  labs(x = "Time (min)", y = "Movement", caption = cap)
```
...
```{r lpr-all-eps-plot-by-plate, fig.height=10}
light_t <- data.table(
  Epoch = c(1, 2, 3),
  Start_t = c(61, 120, 180),
  End_t = c(89, 149, 209)
...
cap <- create.caption(
  "Lines indicate mean movement (raw) by Plate, shading indicates the bootstrapped 95% CIs. Yellow segments at bottom indicate light intervals (vs dark intervals)."
)
...
```{r lsr-by-treatment-plot, eval=F}
...
SR <- paste0("t", 0:9)
behav5d <- copy(lsr.data)
sub_chemids <- unique(lsr.data$chemical.id)
control_conc <- 0
tab <- data.frame()
for (vals in sub_chemids) {
  headers <- c(
    "chemical.id", "conc", "AUC.Pval", "AUC", "PercDiff", "Peak.no", "Peak_dist", "Peak.pval", "PeakDiff"
    )
  # dat <- behav5d %>% filter(chemical.id == vals) %>% select(chemical.id, conc, plate, well,SR)
  dat <- behav5d[chemical.id == vals, .SD, .SDcols = c("chemical.id", "conc", "plate", "well", SR)]
  dat[, conc := as.numeric(conc)]
  cc <- unique(dat$conc)
  # c1 <- dat[which(dat$conc == control_conc),]
  c1 <- dat[conc == control_conc]
  c1 <- c1[complete.cases(c1)]
  # c1_aucs <- apply(c1[,match(c(SR), names(c1))], 2, trapz)
  c1_aucs <- c1[
    , 
    lapply(.SD, function(col) caTools::trapz(x = 1:nrow(c1), y = col)), 
    .SDcols = SR
    ] %>% unlist()
  c_mean <- c1[, lapply(.SD, mean), .SDcols = SR] %>% unlist()
  ...
  for (c in cc) {
    ...
    if (nrow(t) > 1) {
      t_aucs <- t[
        , 
        lapply(.SD, function(col) caTools::trapz(x = 1:nrow(t), y = col)), 
        .SDcols = SR
        ] %>% unlist()
...
stats_table_LSR <- stats_table_LSR[c(
  "chemical.id", "conc", "AUC", "PercDiff",
...
ggplot(
  stats_table_LSR[which(stats_table_LSR$chemical.id == low), ],
  ...
  scale_color_manual(values = c(
    "black", "#CC6666", "#9999CC", "cornflowerblue",
...
```{r lsr-by-treatment-plot-myVersion}
lsr.pattern <- "behavior_5d_FOR_GUI_LSR"
lsr.path <- file.path(dirs, "GUI files", "RemovedAffected")
lsr.files <- list.files(path = lsr.path, pattern = lsr.pattern, full.names = T)
lsr.data <- do.call("rbind", lapply(lsr.files, read.csv)) %>% as.data.table()
...
SR <- paste0("t", 0:9)
dat <- lsr.data[, .SD, .SDcols = c("Treatment", "plate", "well", SR)]
dat.melt <- melt(dat, id.vars = c("Treatment", "plate", "well"))
dat.melt[, variable := as.numeric(gsub("t", "", variable))]
aucs <- dat.melt[, .(AUC = trapz(variable, value)), by = c("Treatment", "plate", "well")]
cap <- create.caption(
  "A plot of the AUCs (per well, not per timepoint) for each of the three Treatments for the LSR analysis. The statistics as currently conducted report no significant differences, but they are based off of calculating the AUCs differently (per timepoint, not per well)"
)
ggplot(aucs, aes(x = Treatment, y = AUC)) + 
...
This is not actually how the LSR data is reported according to the current SARL workflow, but I think this makes more sense, and we can talk about why.
